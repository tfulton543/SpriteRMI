...
/**
 * This class is a server application for a sprite game. It allows users to
 * connect via RMI and create sprite objects which will bounce around in a box.
 * The server will save sprites to a database and read them back in the next
...
public class SpriteGameServer {
	...
	/**
	 * This list holds the sprite objects that are created by clients and read
	 * in from the database
	 */
	private static ArrayList<Sprite> spriteList = new ArrayList<Sprite>();

	/**
	 * This is the reference to the hibernate Session which is used to perform
	 * database I/O
	 */
	...
	public static void main(String[] args) throws RemoteException,
		...
		final StandardServiceRegistry hibernateRegistry = new StandardServiceRegistryBuilder()
				.configure().build();
		MetadataImplementor meta = (MetadataImplementor) new MetadataSources(
				hibernateRegistry).addAnnotatedClass(Sprite.class)
				.buildMetadata();
		...
		// we should fall into this catch block if the tables do not exist on
		// the database (first time the server is run)
		catch (SQLGrammarException e) {
			// create the tables
			System.out
					.println("Database tables not found... Creating tables...");
	...
	/**
	 * This method instantiates the availableColours array and adds all the
	 * colours to it.
	 */
	...
	/**
	 * This method loads all the Sprites that have been persisted in the
	 * database It adds them all to the local ArrayList<Sprite> and should be
	 * called upon startup
	 */
	private void loadSprites() {
		...
		// here we give the server admin the option to clear the database tables
		// (effectively start a new game)
		if (JOptionPane.showConfirmDialog(null,
				"Would you like to start a new game?") == JOptionPane.OK_OPTION) {
	...
	// infinite loop which infinitely moves the sprite objects and then saves
	...
	/**
	 * This method defines the logic which is responsible for moving the
	 * sprites. It moves a single sprite around the play area, and bounces it
	 * off the walls as necessary.
	 * 
	 * @param sprite
	 *            Sprite object that you wish to move
	 */
